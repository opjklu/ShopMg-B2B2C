...
export default {
  ...
  methods: {
    ...
    // 获取一级菜单
    getTopClass() {
      this.HTTP(this.$httpConfig.getClass, { id: 0 }, "post")
        .then(res => {
      ...
      if (
        ...
        if (this.management.length === 0) {
          ...
          for (let index = 0; index < this.management.length; index++) {
            if (
              Number(this.management[index].three_class.id) ==
              Number(this.classItem.three_class.id)
            ) {
              add = false;
    ...
    submit() {
      ...
      this.HTTP(this.$httpConfig.perfectInfo, this.params, "post")
        .then(res => {
