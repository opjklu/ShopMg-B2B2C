<template>
	<div class="account">
		...
		<div class="center">
				...
				<div class="bottom">
					...
					<div class="dianpu" v-for="(item, index) in buyData.store" :key="index">
						...
						<ul class="order-list" v-for="(d,i) in setName" :key="i">
							...
							<li class="order-info" v-if="ite.store_id == item.id && ite.package_id == i" v-for="(ite,inde) in buyData.goods" :key="inde">
			...
			<div class="jiesuan">
				<p><span>{{total_num}} </span>件商品，商品总金额：￥{{Number(total_price)|keep2Num}}</p>
				<p>积分：-￥0.00</p>
				<p>优惠券：-￥0.00</p>
				<p>运费：￥{{freight_price|keep2Num}}</p>
			</div>
			<div class="pay">
				<p class="yingfu r">应付金额：<span>￥{{(Number(total_price) + Number(freight_price))|keep2Num}}</span></p>
				<div class="r">
	...
	export default {
		...
		methods: {
			...
			placeOrder() {
				...
				this.HTTP(this.$httpConfig.orderBeginByCart, {
			...
			getcartOrder() {
				this.HTTP(this.$httpConfig.toSettleAccounts, {
					id: sessionStorage.getItem('cart_id')
				}, 'post').then((res) => {
					this.buyData = res.data.data;
					let goodsList = this.buyData.goods;
					for (const key in goodsList) {
						this.total_price += parseInt(goodsList[key].goods_num) * parseFloat(goodsList[key].goods_discount);
						this.setName[goodsList[key].package_id] = goodsList[key].package_title; //重新整理套餐名字数据
