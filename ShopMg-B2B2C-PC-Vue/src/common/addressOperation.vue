    ...
    <div class="dizhi">
      ...
      <ul class="dizhiul">
        <li
          class="l"
          @click="hit(index)"
          v-for="(sites,index) in addressList"
          :key="index"
          :class="{hover:sites.status == 1}"
        >
          ...
          <p>
            <span @click.stop="modify(sites.id)">修改</span>
          </p>
        </li>
      </ul>

      <p class="guanli">
        <button @click.stop="newly">使用新地址</button>
        <button @click="managementAddress">管理收货地址</button>
      </p>
    </div>
    <el-dialog title="新增收货地址" :is-show="isopen" transition="fadeDown" :visible="isopen" @close="isopen = false">
      <div class="contents">
        <div>
          <p class="l">
            <span class="l">*</span>所在地址：
          </p>
          <p class="l">
            <selection-area
              v-if="isShow"
        ...
        <div>
          <p class="l">
            <span class="l">*</span>详细地址：
          </p>
          <textarea
            v-model="addressInfo.address"
            class="l"
            placeholder="建议您如实填写详细收货地址，例如街道名称，门牌号码，楼层和房间号码等信息"
          ></textarea>
        </div>
        ...
        <div>
          <span>*</span>收货人姓名：
          <input type="text" v-model="addressInfo.realname" />
        </div>
        <div>
          <span>*</span>手机号码：
          <input type="text" v-model="addressInfo.mobile" />
        </div>
        <!-- <div>电话号码：<input type="text" v-model="mobile"/></div> -->
        <p class="moren">
          <input type="checkbox" v-model="addressInfo.status" />设为默认收货地址
        </p>
...
export default {
  ...
  methods: {
    ...
    getAddressList() {
      this.HTTP(this.$httpConfig.addressLists, {}, "post", false).then(res => {
