    ...
    <div class="middle center">
      ...
      <div class="l miaoshu" @click="fun" :class="{jiantou:up}">
        <p class="l">
          描述
          <br />
          <span>{{(Number(storeInfo.desccredit))|keep2Num}}</span>
        </p>
        <p class="l">
          服务
          <br />
          <span>{{(Number(storeInfo.servicecredit ))|keep2Num}}</span>
        </p>
        <p class="l">
          发货
          <br />
          <span>{{(Number(storeInfo.deliverycredit))|keep2Num}}</span>
        </p>
      </div>
      <div class="search l">
        <div class="l top">
      ...
      <div class="shoucang" v-show="shoucang">
        ...
        <div class="right l">
          ...
          <div class="l bottom">
            ...
            <p>
              开店时间：
              <span v-show="storeInfo.update_time">{{storeInfo.create_time | formatDate}}</span>
            </p>
...
export default {
  data() {
    return {
      ...
      on: "",
      isactive: "",
      arr0: ["GTX", "CPU", "Memory", "Computer", "Phone", "卫衣", "珠宝"],
      nav: [
        {
          id: "0",
          name: "首页",
          url: this.$constant.s_url + "storehome?id=" + this.$route.query.id
        }
      ],
      isactives: "",
      shoucang: "",
  ...
  methods: {
    ...
    SearchOurStore() {
      ...
      this.$parent.searchGoodsByThisStore(this.search_content);
    },
    //搜全站
    SeekTotalStation() {
    ...
    // 获取店铺信息
    getStoreInfo() {
      this.HTTP(this.$httpConfig.getStoreDetails, { id: this.storeID }, "post")
        .then(res => {
    ...
    // 获取菜单导航
    getStoreNav() {
      this.HTTP(this.$httpConfig.getStoreNav, { id: this.storeID }, "post")
        .then(res => {
          console.log(res);
          var list = res.data;
          for (var i in list) {
    ...
    collectStore(type) {
      if (type) {
        //关注
        this.HTTP(this.$httpConfig.setAttenStore, { id: this.storeID }, "post")
          .then(res => {
...
.middle {
  ...
  .miaoshu {
    ...
    width: 120px;
    cursor: pointer;
    background: url(../assets/img/downjiantou.jpg) no-repeat top 13px right 0;
    p {
      font-size: 12px;
      margin-left: 9px;
      color: #a0a0a0;
