    ...
    <div class="center">
      ...
      <div class="jiesuan">
        <p>
          ...
          件商品，商品总金额：￥{{computedTotal(data)|keep2Num}}
        </p>
        <p>积分：-￥0.00</p>
        <p>优惠券：-￥0.00</p>
      ...
      <div class="pay">
        <p class="yingfu r">
          应付金额：
          <span>￥{{computedTotal(data) | keep2Num}}</span>
        </p>
        <div class="r">
          <p class="l">
            寄送至：
...
export default {
  ...
  methods: {
    ...
    mealCar() {
      this.HTTP(
        this.$httpConfig.cartPackageBuyNow,
        {
        ...
        .then(res => {
          ...
          this.storeData = res.data.store[0];
          let { values } = Object;
          this.data = values(res.data.goods);
    ...
    toDetails() {
      window.open(
        window.location.origin + "/inroyaldrink?id=" + this.goodsData.id
      );
    },
    toStore() {
