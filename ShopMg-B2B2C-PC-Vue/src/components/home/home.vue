  ...
  methods: {
    ...
    getHome() {
      ...
      this.HTTP(this.$httpConfig.home, {}, "post", false)
        .then(res => {
          console.log(res);
          this.items = res.data.announcement;
    ...
    async getFloor() {
      if (this.floorIndex < 13) {
        await this.HTTP(
          ...
          .then(res => {
            //判断数组是否为空，空则假数据
            if (res.data.middle.length === 0) {
              res.data.imgArray = this.imgArray;
            } else {
              res.data.imgArray = res.data.middle;
            } //console.log(this);
            let floor = res.data;
