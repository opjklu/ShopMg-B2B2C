<template>
  <div class="myOrder l">
    <div>
      <div class="top">
        <ul class="l">
          <li @click="tab(index)" class="l" v-for="(item,index) in myul" :class="{li:isli==index}" :key="index"><span class="line">{{item.li}}<span class="huang">{{item.spa}}</span></span>
          </li>
        </ul>
  ...
  export default {
    ...
    methods: {
      tab(index) {
        console.log(index)
        if (!index) {
          this.HTTP(this.$httpConfig.integralList+/p/+this.currentPage, {
          }, 'post', false).then(res => {
            this.data = res.data
            console.log(~~this.data.count, res.data.count)
